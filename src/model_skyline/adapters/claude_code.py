"""Conservative adapter for Claude Code ``--output-format json`` results.
...
content-bearing fields into the canonical row or into an exception.
...
CLAUDE_CODE_ADAPTER_VERSION: Final = "1"
...
MAX_CLAUDE_CODE_JSON_STRUCTURAL_TOKENS: Final = 100_000
MAX_CLAUDE_CODE_JSON_NUMBER_CHARACTERS: Final = 1_024
...
_USAGE_FIELDS = frozenset(_USAGE_COUNT_FIELDS) | {
    "server_tool_use",
    "service_tier",
    "cache_creation",
    "inference_geo",
...
_SERVER_TOOL_USE_FIELDS = frozenset({"web_search_requests", "web_fetch_requests"})
_CACHE_CREATION_FIELDS = frozenset({"ephemeral_1h_input_tokens", "ephemeral_5m_input_tokens"})
...
class ClaudeCodeRouteMapping(FrozenModel):
    """Bind one complete Claude Code invocation to an exact offering.

    The attestation covers every main-agent, subagent, fallback, and internal
    model call represented by the cumulative result. It also supplies model,
    provider, and stable pricing-basis identity when the CLI's optional
    ``canonicalModel`` or ``provider`` fields are absent.
    """

    offering: OfferingKey
    model_usage_key: str
    upstream_provider: str
    expected_usage_service_tier: str
    expected_usage_inference_geo: str
    expected_usage_speed: str
    ...
    def route_is_content_free_and_explicit(self) -> ClaudeCodeRouteMapping:
        ...
        _usage_route_value(
            self.expected_usage_service_tier,
            field="expected_usage_service_tier",
        )
        _usage_route_value(
            self.expected_usage_inference_geo,
            field="expected_usage_inference_geo",
            allow_empty=True,
        )
        _usage_route_value(self.expected_usage_speed, field="expected_usage_speed")
...
def _validate_json_structure(raw: bytes) -> None:
    """Bound JSON allocation before UTF-8 decoding and tree construction."""
    ...
    for byte in raw:
        ...
        if in_number:
            if byte in number_body:
                number_characters += 1
                if number_characters > MAX_CLAUDE_CODE_JSON_NUMBER_CHARACTERS:
                    raise ClaudeCodeAdapterError(
                        "Claude Code JSON numeric token exceeds the character limit"
                    )
                continue
            in_number = False
            number_characters = 0
        ...
        if byte in openers:
            ...
            if depth > MAX_CLAUDE_CODE_JSON_DEPTH:
                raise ClaudeCodeAdapterError("Claude Code JSON exceeds the nesting limit")
        elif byte in closers and depth:
            depth -= 1
        if byte in structural:
            structural_tokens += 1
            if structural_tokens > MAX_CLAUDE_CODE_JSON_STRUCTURAL_TOKENS:
                raise ClaudeCodeAdapterError("Claude Code JSON exceeds the structural token limit")
...
def _usage_route_value(value: Any, *, field: str, allow_empty: bool = False) -> str:
    if allow_empty and value == "":
        return ""
    if (
        not isinstance(value, str)
        or _OPAQUE_IDENTIFIER_RE.fullmatch(value) is None
        or _CREDENTIAL_RE.search(value) is not None
    ):
        raise ClaudeCodeAdapterError(f"{field} must be a content-free upstream usage dimension")
...
def _validate_terminal_shape(result: Mapping[str, Any]) -> str:
    ...
    if not isinstance(subtype, str) or subtype not in _TERMINAL_RESULT_SUBTYPES:
    ...
    _count(result["duration_ms"], field="duration_ms")
    _count(result["duration_api_ms"], field="duration_api_ms")
    _count(result["num_turns"], field="num_turns")
    _usd(result["total_cost_usd"], field="total_cost_usd")
...
def _core_usage(
    ...
    *,
    ...
    if set(usage) != _USAGE_FIELDS:
        raise ClaudeCodeAdapterError("Claude Code usage does not match the reviewed schema")
    counts = {field: _count(usage[field], field=f"usage.{field}") for field in _USAGE_COUNT_FIELDS}

    dimensions = (
        ("service_tier", route.expected_usage_service_tier),
        ("inference_geo", route.expected_usage_inference_geo),
        ("speed", route.expected_usage_speed),
    )
    for field, expected in dimensions:
        actual = _usage_route_value(usage[field], field=f"usage.{field}", allow_empty=True)
        if actual != expected:
            raise ClaudeCodeAdapterError(
                f"Claude Code usage {field} does not match the route mapping"
            )

    server_tools = usage["server_tool_use"]
    if not isinstance(server_tools, Mapping) or set(server_tools) != _SERVER_TOOL_USE_FIELDS:
        raise ClaudeCodeAdapterError(
            "Claude Code usage server_tool_use does not match the reviewed schema"
        )
    counts["web_search_requests"] = _count(
        server_tools["web_search_requests"],
        field="usage.server_tool_use.web_search_requests",
    )
    web_fetch_requests = _count(
        server_tools["web_fetch_requests"],
        field="usage.server_tool_use.web_fetch_requests",
    )
    if web_fetch_requests != 0:
        raise ClaudeCodeAdapterError(
            "Claude Code usage web_fetch_requests is unsupported by the reviewed aggregate"
        )

    cache_creation = usage["cache_creation"]
    if not isinstance(cache_creation, Mapping) or set(cache_creation) != _CACHE_CREATION_FIELDS:
        raise ClaudeCodeAdapterError(
            "Claude Code usage cache_creation does not match the reviewed schema"
        )
    for field in _CACHE_CREATION_FIELDS:
        if _count(cache_creation[field], field=f"usage.cache_creation.{field}") != 0:
            raise ClaudeCodeAdapterError(
                "Claude Code usage retention-specific cache creation is unsupported"
            )
    if usage["iterations"] != []:
        raise ClaudeCodeAdapterError(
            "Claude Code usage iterations does not match the reviewed aggregate"
        )
    return counts
...
def _single_model_usage_entry(
    ...
    *,
    ...
    if not isinstance(entry, Mapping) or not set(entry).issubset(_MODEL_USAGE_FIELDS):
...
def _model_usage(
    ...
    *,
    route: ClaudeCodeRouteMapping,
    core: Mapping[str, Decimal],
) -> tuple[dict[str, Decimal], Decimal]:
    entry = _single_model_usage_entry(result, route=route)
    if entry is None:
        raise ClaudeCodeAdapterError("Claude Code modelUsage is required")
    ...
    if model_cost != total_cost:
        raise ClaudeCodeAdapterError("Claude Code single-model cost does not match total_cost_usd")
...
def adapt_claude_code_result_json(
    ...
    *,
    ...
    if not work_unit_success.is_finite() or not Decimal(0) <= work_unit_success <= Decimal(1):
        raise ClaudeCodeAdapterError("work_unit_success must be finite and between zero and one")
    ...
    core = _core_usage(result, route=route)
    model_usage = result["modelUsage"]
    ...
    if accounting_unavailable:
        _single_model_usage_entry(result, route=route)
        counts = None
        cost = None
    else:
        counts, cost = _model_usage(result, route=route, core=core)
