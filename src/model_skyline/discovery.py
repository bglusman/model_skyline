...
def parse_openrouter(
    ...
    try:
        ...
        raise DiscoveryError("model pattern is not a valid regular expression") from exc
    ...
    *,
    ...
    try:
        for url, kind, limit in (
            ...
            found = (
                parse_openrouter(raw, source, model_pattern=model_pattern)
                if kind == "catalog"
                else parse_feed(raw, source)
            )
